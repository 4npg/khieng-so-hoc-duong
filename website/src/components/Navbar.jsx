...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass-panel border-b border-slate-800/80 bg-[#0a0d18]/80">
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className="md:hidden glass-panel border-b border-slate-800 px-4 pt-2 pb-4 space-y-2 text-sm font-medium">
          <a href="#hero" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Trang chủ</a>
          <a href="#video" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Phim ngắn Tuyên truyền</a>
          <a href="#scams" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Cảnh báo Lừa đảo</a>
          <a href="#posters" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Tải Ấn phẩm AI</a>
          <a href="#quiz" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Trắc nghiệm An toàn số</a>
          <a href="#faq" onClick={() => setIsOpen(false)} className="block py-2 text-slate-200 hover:text-cyan-400">Hỏi đáp Pháp luật</a>
        </div>
      )}
    </nav>
  );
